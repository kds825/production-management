...
import { GlossaryModal } from "./decision/GlossaryModal";
...
export function DecisionConstraintsModal() {
  ...
  // 용어집 sub-modal 토글 — 본 모달 위에 z-index 한 단계 더 높게 마운트.
  ...
  return (
    <>
      ...
      >
        <div
          className="bg-white rounded-lg shadow-xl w-[960px] max-w-[95vw] max-h-[88vh] overflow-hidden flex flex-col"
        ...
        >
          ...
          <div className="flex items-center justify-between border-b px-6 py-4 shrink-0">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="text-xs font-medium px-2.5 py-1 rounded border border-gray-300 text-gray-600 hover:bg-gray-50"
              >
                용어집
              </button>
              <button
          ...
          <div className="flex-1 overflow-auto px-6 py-5 flex flex-col gap-5">
            ...
            {status === "ok" && data && (
              <>
                {/* ── Group 1: Context ─────────────────────────────────
                  수주의 비즈니스 정보. matchingTask 없으면 라벨도 생략. */}
                {matchingTask && (
                ...
                {/* ── Group 2: Narrative ───────────────────────────────
                  한 줄 판단. context 와 evidence 사이 bridge. */}
                <div className="text-xs font-semibold text-gray-700 uppercase border-t border-gray-200 pt-4">
                  요약
                </div>
                ...
                {/* ── Group 3: Evidence ────────────────────────────────
                  배정 슬롯, 활성 제약, 대안, 가중치 기여, 수동 조정 — 솔버
                  결정을 뒷받침하는 사실 근거. */}
                <div className="text-xs font-semibold text-gray-700 uppercase border-t border-gray-200 pt-4">
                  근거
                </div>
                  ...
                  {data.contributions.length === 0 ? (
                    ...
                    (() => {
                      const sorted = [...data.contributions].sort(
                        (a, b) =>
                          Math.abs(b.weight_applied) -
                          Math.abs(a.weight_applied),
                      );
