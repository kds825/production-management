...
rowTasks.map((task) => {
  ...
  const overlay =
    ...
    diff?.kind === 'moved' ? 'moved' : undefined;
  ...
  return (
    <>
      ...
      {diff?.kind === 'moved' && compareMode.filters.moved && (
        <GanttTaskBlock
          task={{ ...task, start: diff.old_start, end: diff.old_end,
...
- 출력: `Map<string, DiffIndexEntry>`
  ...
  type DiffIndexEntry =
    ...
    | { kind: 'moved';  task: MovedTask;   old_start: Date; old_end: Date; old_equipment: string; start_delta_hours: number; equipment_changed: boolean }
...
이 스펙이 승인되면 `writing-plans` skill 로 넘어가 implementation plan 을 작성한다.
