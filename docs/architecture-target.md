...
- [✓] Phase 5 §9.2 — lex vs weighted-sum 실측 dual-run 완료. docs/lex-mode-comparison.md §6 에 결과 표 추가 — `run_label=20260425_225331` (985 batch, soft constraints) 기반: lex 가 wall-time 158× 빠름 (0.19s vs 30.04s), OPTIMAL vs FEASIBLE, 2 group 더 placement, 시스 cluster 보존 양쪽 100%, 캘린더 그리디 makespan 동치 (38400 min). schedule_task drift: 96 common groups 중 equipment 5 / start_datetime 52 변동. 산출물: /tmp/lex_dual_run.json + /tmp/lex_dual_run.py.
- [✓] Phase 5 §9.1 — 실 ERP 양 서버 dual-run 완료. `Documents/ERP생산계획_v1.xls + SM재고리스트.xls` 기반 main*baseline (port 8001, cb3ce08) 와 refactoring (port 8000) sequential stage1+stage2 비교 — **회귀 0** (equipment / start_datetime / end_datetime 모든 변경 0건, 94 common batch_groups). refac 만 1 group 추가 placement (`A100*회\_2026W17H1_95SQ_REMAIN`— refactoring 의 개선분). violations: main 169 / refac 173. **Side note**: main_baseline 의`\_purge_run_data` 가 sales_order ↔ wip_inventory ↔ production_batch FK cycle 처리 누락 회귀 (refac 가 fix 한 patch 와 다름) — dual-run 시 manual cleanup 으로 우회. 산출물: /tmp/erp_dual_run_diff.md + /tmp/erp_dual_run.py.
- [✓] Phase 5 §9.3 — gstack/QA UI walkthrough 완료. frontend (Next.js, port 3000) 의 4 메인 페이지 (`/scheduler` Gantt / `/plan-register` Stage 1 업로드 / `/scheduling-review` Stage 2 결과 검토 / `/audit` Decision Card) 모두 정상 렌더링 + **콘솔 에러 0** 확인. dual-run 후 데이터 (run_label `20260426_165811`, 985 배치) 기반 검증: Gantt 91 납기 초과 표시, scheduling-review 9 공정 탭 + SM 재고 가용 8건, audit 91 결정 기록 (Stage 2: 91 CP-SAT 순서 → 설비 매핑) 모두 표시. PwC samildevkit 디자인 시스템 준수 확인 (Orange/Red 컬러, 사이드바 navigation). 산출물: /tmp/qa_01_scheduler.png, /tmp/qa_02_plan_register.png, /tmp/qa_03_scheduling_review.png, /tmp/qa_04_audit.png, /tmp/qa_05_audit_selected.png.
