...
- [✓] Phase 5 §9.1 — 실 ERP 양 서버 dual-run 완료. `Documents/ERP생산계획_v1.xls + SM재고리스트.xls` 기반 main_baseline (port 8001, cb3ce08) 와 refactoring (port 8000) sequential stage1+stage2 비교 — **회귀 0** (equipment / start_datetime / end_datetime 모든 변경 0건, 94 common batch_groups). refac 만 1 group 추가 placement (`A100_회_2026W17H1_95SQ_REMAIN` — refactoring 의 개선분). violations: main 169 / refac 173. **Side note**: main_baseline 의 `_purge_run_data` 가 sales_order ↔ wip_inventory ↔ production_batch FK cycle 처리 누락 회귀 (refac 가 fix 한 patch 와 다름) — dual-run 시 manual cleanup 으로 우회. 산출물: /tmp/erp_dual_run_diff.md + /tmp/erp_dual_run.py.
