...
File: `scripts/seed_weight_constraints.py`. Idempotent (skip if row exists). Each new row has `implementation_type='code_logic'` and `params_json={"weight": current_value}`. (Values `code_logic` / `hybrid` / `table_param` are the canonical vocabulary per DB reality — see "Known compromises" `implementation_type vocabulary drift` resolution.)
...
- **ConstraintConfig.implementation_type vocabulary — RESOLVED 2026-04-23**: Spec §7 + §1 falsification table aspirationally defined `implementation_type` values `{solver_term, pre_filter, post_filter}`. Supabase rows actually use `{code_logic, hybrid, table_param}` (33 active rows inspected via load_active_constraints smoke in Task 2A.1). **Resolution: DB wins (option a).** `code_logic` = constraint posted as CP-SAT term; `hybrid` = both solver-side and pre/post logic; `table_param` = pure params_json lookup (no solver branch). Canonical `Literal["code_logic", "hybrid", "table_param"]` from now on. Plan Task 5A.2 seed corrected (`implementation_type='code_logic'`). Task 2A.2 model_builder does NOT branch on implementation_type (Week 2 preserves current behavior — solver still doesn't read the field), so this resolution is forward-looking for Week 4 narrator categorization + Week 5 weight migration.
