...
from app.domain.constants import (
    PREDECESSOR_PROCESS,  # noqa: F401  re-export until Week 9 (D7-C)
    PROCESS_ORDER,
    _DEFAULT_WELDING_MIN,  # noqa: F401  re-export until Week 9 (D7-C)
    _WIP_SKIP_PROCESSES,  # noqa: F401  re-export until Week 9 (D7-C)
)
from app.services.calendar_engine import (  # noqa: F401
    calculate_end_datetime,
    calculate_start_datetime,  # re-exported for slot_filters & test monkeypatch
)

# apply_jit_delay 는 greedy.auto_schedule._run_optimization_once 가 사용하지만,
# 기존 테스트(`test_jit_integration`) 가 schedule_optimizer 모듈에서
# `patch.object(schedule_optimizer, "apply_jit_delay", ...)` 로 mock 한다.
# Week 3 Task 3A.2 이동 후에도 monkeypatch 가 유효하도록 모듈 namespace 에 노출.
...
# _is_core_group, _st_sq, _is_sheath_group, _extract_core_main_sq 는
# app.services.scheduling_shared.group_ops 로 이동 (Week 3 Task 3A.1).
# 아래 import 가 모듈 namespace 에 re-export (D7-C).
...
# auto_schedule, _tardiness_boost_retry, _purge_run_tasks, _run_optimization_once
# 등 그리디 핵심 함수와 보조 헬퍼 (_should_apply_jit, _SHEATH_ROUTING,
# _group_earliest_due, _sheath_group_color_rank, _sheath_group_due_week_int,
# _get_tp_line_speed, _get_sheath_type) 는 app.services.greedy.auto_schedule 로
# 이동 (Week 3 Task 3A.2). 아래 import 가 모듈 namespace 에 re-export (D7-C, Week 9).
#
# 이동된 코드의 행위 표지 (회귀 검출 anchor; test_tp_routing 등이 본 파일을
# source-string 으로 검사하므로 이전 위치를 가리키는 marker 를 남긴다):
...
from app.services.greedy.auto_schedule import (  # noqa: E402, F401
...
# _schedule_multi_equipment 는 app.services.scheduling_shared.group_ops 로 이동
...
# _find_eligible_equipment, _narrow_by_stranding, align_start_to_predecessor_end 은
# app.services.scheduling_shared.slot_filters 로 이동 (Week 3 Task 3A.1).
# 아래 import 가 모듈 namespace 에 re-export (D7-C invariant).
from app.services.scheduling_shared.slot_filters import (  # noqa: E402, F401
...
# _find_available_slot 은 app.services.greedy.slot_finder 로 이동
# (Week 3 Task 3A.2). 아래 import 가 모듈 namespace 에 re-export (D7-C, Week 9).
from app.services.greedy.slot_finder import (  # noqa: E402, F401
    _find_available_slot,  # re-export shell (D7-C, Week 9)
)


# _get_drum_winding_min, _get_stranding_setup_min 은
# app.services.scheduling_shared.group_ops 로 이동 (Week 3 Task 3A.1).
# 아래 import 가 모듈 namespace 에 re-export (D7-C).
from app.services.scheduling_shared.group_ops import (  # noqa: E402, F401
...
# _get_tp_line_speed, _get_sheath_type 는 app.services.greedy.auto_schedule 로
# 이동 (Week 3 Task 3A.2). 위쪽 그리디 패키지 import 블록에서 이미 re-export 됨.

# _filter_by_sheath_routing 은 app.services.scheduling_shared.slot_filters 로
# 이동 (Week 3 Task 3A.1). 아래 import 가 모듈 namespace 에 re-export.
# 이동된 함수는 _SHEATH_ROUTING / _get_sheath_type 을 지연 import 로 가져오며,
# 그 두 심볼은 위 greedy.auto_schedule import 블록을 통해 모듈 namespace 유지.
from app.services.scheduling_shared.slot_filters import (  # noqa: E402, F401
